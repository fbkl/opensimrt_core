		...
		void computeControls(const SimTK::State& s, SimTK::Vector& controls) const override
		{
			if (from_frame )
			{
				if (to_frame)
				{
					...
					std::cout << "SimTK::Rotation SimTK::Matrix:" << std::endl;
					std::cout << rotationTranslation.R() << std::endl;
					std::cout << "Translation SimTK::Vector:" << std::endl;
...
int main() {
	...
	std::string actu_name = "biceps";
	brain->prescribeControlForActuator(actu_name, StepFunction(0.5, 3, 0.3, 1));
	...
	PhysicalOffsetFrame* radiusCenter = new PhysicalOffsetFrame(
			"radiusCenter", *radius, SimTK::Transform(SimTK::Vec3(0, 0.5, 0)));
