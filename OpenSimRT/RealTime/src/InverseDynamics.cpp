...
SimTK::Vector ExternalWrench::Input::toVector() const {
    SimTK::Vector out(9);
    out[0] = point[0];
    out[1] = point[1];
    out[2] = point[2];
...
void ExternalWrench::Input::fromVector(const SimTK::Vector& in) {
    point[0] = in[0];
    point[1] = in[1];
    point[2] = in[2];
    force[0] = in[3];
...
void ExternalWrench::computeForce(const SimTK::State& state,
    ...
    SimTK::Vec3 torque = input.torque;
...
ExternalWrench::getWrenchFromStorage(double t, const vector<string>& labels,
    ...
    SimTK::Vector row(columns.getSize() - 1, 0.0);
    storage.getDataAtTime(t, columns.getSize() - 1, row);

    SimTK::Vector collect(columns.getSize() - 1, 0.0);
